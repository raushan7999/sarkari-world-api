-- Adapted from the Node service's 0005_align_to_trimmed_schema.sql, which is
-- kept verbatim at reference/0005_align_to_trimmed_schema.upstream.sql. The
-- only difference is the ArticleCategory re-homing CASE, which the original
-- invites tuning ("change the CASE below to re-bucket differently").
--
-- The original sends both 'job' and 'exam' to latest_job; that would leave the
-- Result / Admit Card / Answer Key / Syllabus / Admission pages empty, since
-- every article for them sat in the legacy 'exam' bucket.
--
